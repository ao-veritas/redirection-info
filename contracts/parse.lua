...
function getUserActivityData(fullData, userIdKey, tagActionKey, actions)
    ...
    for date, counts in pairs(userActivity) do
        local entry = { date = date, total = #counts.total, addresses = tableKeys(counts.addresses) }
        for _, action in ipairs(actions) do
            entry[action] = #(counts[action] or {})
        end
        table.insert(result, entry)
    end
    ...
    --             end
    --         end
    --         table.insert(activeUsers, { date = data[i].date, activeUsers = #window })
    --     end
    --     return activeUsers
    -- end
